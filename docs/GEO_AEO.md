...
AI hodnotí důvěru přes **E-E-A-T v éře LLM**: Experience, Expertise, Authoritativeness, Trust. Pro nás to znamená: konkrétní certifikovaná metoda (Bicom Optima, třída IIa, ISO 13485), konkrétní lokalita (Písek), konzistentní fakta napříč webem i mapami, a strukturovaná data.
...
```
Method: Bicom Optima  ──poskytováno v──▶  Place: Písek
```
Implementace: `LocalBusiness` + `Service`/`MedicalProcedure` + `FAQPage` + `Article` + `BreadcrumbList`. Šablony v `02_JSON-LD_sablony.md`.
...
- **Osobním přístupu** — empatie, diskrétnost, individuální péče.
